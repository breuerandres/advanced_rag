...
- Structured output is **incompatible with token streaming** in some model/version combinations, so the text path consumes the provider's `chat_stream` and runs each delta through a custom server-side character parser (`AnswerStreamParser`) that watches for the `"answer":"..."` field and emits its unescaped character deltas as SSE `event: answer-token` **as they arrive from the model** (true time-to-first-token, not a replay of the finished answer); citations are parsed from the completed JSON payload and emitted as one `event: citations`, followed by `event: usage` and `event: done`. The audit row is written after generation completes (exact usage, end-to-end latency). The multimodal path streams the same way: it consumes the provider's `multimodal_stream` (Responses API with `stream=True`) through the **same** `AnswerStreamParser`, so image-grounded answers also emit incremental `answer-token` deltas. Only cache hits and no-results answers emit a single whole-answer `answer-token` (no model generation occurs there). If a future OpenAI version supports streaming structured outputs natively, switch to that.
...
- Multimodal generation uses the OpenAI Responses API with `store: false` and `stream: true`, text context plus selected images as input, and structured JSON output through Responses `text.format`; the streamed `response.output_text.delta` events feed the shared `AnswerStreamParser` token-by-token and `response.completed` carries final usage. Image bytes are converted to in-memory base64 data URLs for the provider request and are never persisted as base64.
