...
- Unified auth uses `__Host-session` as the only browser auth cookie. FastAPI chat/feedback read the `__Host-session` cookie and validate it through `.NET` internal `GET /internal/session/validate` with `X-Internal-Service-Token` on every request that uses `DotnetSessionValidator`.
- FastAPI does not cache `.NET` access-relevant session claims. Raw session cookies must not be logged.
- `.NET` returns hierarchical effective-access claims from internal session validation: `userId`, primary `role`, `isGlobalAdmin`, `organizationalUnitId`, sorted `groups`, `accessScopeVersion`, `accessScopeHash`, and `corpus`. User role, group, organizational-unit, and active-status changes must update `users.access_scope_version` so semantic cache and audit partitioning change with the effective scope.
...
- Document access in the current implementation is group/department-based plus document attributes. Per-user document access exceptions are out of scope. If a customer needs an exception, administrators create a specific group and assign the user to that group.
- Hierarchical access implementation status on 2026-06-08: Task 1 implemented the app-schema hierarchy baseline, and Task 2 implemented the effective-access scope contract used by `.NET` and FastAPI. Later slices still need to replace legacy group-only document lifecycle/API/UI behavior and RAG branch-aware filtering.
- Planned access-model redesign, not yet implemented: document access should move toward a hybrid model with hierarchical organizational units plus transverse groups. Each user has one primary organizational-unit assignment and zero or more group assignments. `Admin` is a global unrestricted system role: it bypasses organizational-unit and group restrictions for management, publishing, chat retrieval, and document viewing. Non-admin users receive explicit scoped document-management permissions instead of relying on the current all-purpose `DocumentManager` role. `DocumentEditor` can create, import, edit, update metadata, send to review, and return/reject draft or in-review documents within the user's management scope. `DocumentPublisher` includes `DocumentEditor` capabilities and can publish, archive, and restore documents within that management scope. The normal management scope is the user's assigned organizational unit plus descendants. Organizational-unit read access is branch-scoped, not sibling-scoped: documents published at an ancestor organizational unit are visible to users in descendant units, and users assigned to a parent organizational unit can read documents assigned to descendant units; sibling branches do not match. Organizational units should be stored with a closure table for hierarchical permission joins: the node table keeps the direct parent relationship, and a closure table stores ancestor, descendant, and depth rows. Documents intended for all company users should be assigned to the root organizational unit, not to an empty rule. A document rule that targets the root organizational unit matches every user's organizational-unit dimension; normal non-root organizational-unit rules require the document unit and user unit to be in the same ancestor/descendant branch. Groups remain for department-like or cross-cutting access that does not fit a strict tree. Groups do not grant document publication authority by themselves. Each transverse group must have an ownership scope, either global or tied to one organizational-unit node. A non-admin publisher may attach a group to a document rule only when the group owner's organizational unit is inside the publisher's management scope, or when an explicit publisher-to-group usage grant allows that publisher to use the group. Global groups require `Admin` or an explicit usage grant; they are not automatically available to every non-admin publisher. Non-admin users must also have scoped document-management permission for the document's organizational-unit rule. Document access rules use AND semantics within a rule and OR semantics between rules: a user can access a document when at least one rule matches, and every dimension configured on that matching rule is satisfied. When a rule includes multiple groups, the group dimension matches if the user belongs to any listed group. A rule with no organizational unit and no group is invalid and must not grant access implicitly. User moves and group changes must change effective access for chat and docs on the next request. The first hierarchy-management slice should allow creating, renaming, and deactivating organizational-unit nodes, but should defer moving branches until a later slice with impact preview and stronger audit. For the current development/demo dataset, this refactor does not require migration from existing flat groups and document permissions; implementation may reset data and seed a fresh hierarchy after an explicit user-owned destructive-data checkpoint. The approved fresh seed hierarchy is `Empresa` as root, with child units `Comunicación`, `Operaciones`, `Recursos Humanos`, `Sistemas`, and `Finanzas`; `Comunicación` has child units `Marketing` and `Producción Audiovisual`. Approved seed transverse groups and ownership scopes are `Gerentes` as global with publishing use only through explicit grants, `Comité de crisis` owned by `Comunicación`, `Liderazgo` as global and admin-only by default, and `Demo viewers` as a global technical group if still needed for tests. Approved seed users are `admin@admin.com` as unrestricted `Admin` at `Empresa` with `Liderazgo`, `manager.comunicacion@demo.com` as a non-admin `DocumentPublisher` scoped to `Comunicación` with `Gerentes` and `Comité de crisis`, `viewer.marketing@demo.com` as `Viewer` at `Marketing`, `viewer.audiovisual@demo.com` as `Viewer` at `Producción Audiovisual`, `viewer.sistemas@demo.com` as `Viewer` at `Sistemas`, and `crisis.comunicacion@demo.com` as `Viewer` at `Marketing` with `Comité de crisis`. Approved seed documents are `Manual general de comunicación interna` with rule `Empresa`, `Guía del área Comunicación` with rule `Comunicación`, `Calendario de campañas de Marketing` with rule `Marketing`, `Checklist de producción audiovisual` with rule `Producción Audiovisual`, `Procedimiento de guardias de Sistemas` with rule `Sistemas`, and `Protocolo de comunicación en crisis` with rule `Comunicación` plus group `Comité de crisis`.

### Planned Hierarchical Access Acceptance Matrix
...
| `manager.comunicacion@demo.com` | Company-wide, `Comunicación`, `Marketing`, `Producción Audiovisual`, and crisis documents. Does not see unrelated `Sistemas` documents unless `Admin` grants broader authority. | Can create, edit, review, publish, archive, and restore documents in `Comunicación`, `Marketing`, and `Producción Audiovisual`. Can use `Comité de crisis` because it is owned by `Comunicación`. Cannot publish `Empresa`, `Sistemas`, `Finanzas`, or `Liderazgo` documents. Cannot use `Gerentes` for publishing unless an explicit grant is added. |
...
Acceptance cases must also prove that group membership alone does not grant publication authority, `Gerentes` is not usable for publishing without an explicit grant, empty document-access rules are rejected, and user node/group changes affect chat/docs authorization on the next request.
...
In the planned hierarchical access refactor, `Admin` document-viewer authorization is global and unrestricted. Non-admin document-viewer authorization is evaluated from the user's organizational-unit scope plus groups against the document's access rules.
...
- Planned hierarchical access refactor: replace the single non-admin `DocumentManager` lifecycle authority with scoped `DocumentEditor` and `DocumentPublisher` document-management permissions. `DocumentPublisher` may publish only documents whose organizational-unit rule is within the user's management scope and whose group dimensions the user is allowed to use. `Admin` remains unrestricted.
