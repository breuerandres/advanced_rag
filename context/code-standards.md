...
- FastAPI chunking may index accessible text from document images already present in saved HTML (`alt`, `aria-label`, `title`, and captions). It may store FastAPI-owned `chunk -> image_id` references for stable app-controlled image URLs, but it must not index raw image URLs, write document image metadata, expose object keys, or call raw object storage directly.
- Query-time multimodal RAG must fetch image bytes only through `.NET` internal endpoints protected by `X-Internal-Service-Token`, apply configured caps before sending provider requests, and avoid caching multimodal answers until the cache key includes visual evidence identity and image processing settings.
